"""Jointly overfit a deterministic, non-overlapping SAR patch set."""
...
DEFAULT_ANCHOR = "patch_row_17500_col_9400_2.mat"
METRIC_NAMES = (
...
def patches_overlap(
    first: DiscoveredPair,
    second: DiscoveredPair,
    patch_shape: tuple[int, int],
) -> bool:
    """Return whether two source rectangles overlap by at least one pixel."""

    height, width = patch_shape
    return abs(first.row - second.row) < height and abs(first.col - second.col) < width


def select_spatially_distributed_pairs(
    ...
    *,
    ...
    """Select an anchor followed by deterministic normalized farthest points."""

    if sample_count < 2:
        raise ValueError("sample_count must be at least 2")
    if len(pairs) < sample_count:
        raise ValueError(
            f"dataset has only {len(pairs)} pairs, fewer than sample_count={sample_count}"
        )
    anchors = [pair for pair in pairs if pair.echo_path.name == anchor_filename]
    if len(anchors) != 1:
        raise ValueError(
            f"anchor filename must identify exactly one pair: {anchor_filename!r}; "
            f"matches={len(anchors)}"
        )

    row_values = [pair.row for pair in pairs]
...
def load_selected_samples(
    ...
    *,
    ...
    for pair in pairs:
        ...
        inputs, targets, scale = normalize_complex_pair(
            echo,
            image,
            epsilon=rms_epsilon,
        )
        target_peak = float(np.abs(image / scale).max())
        if target_peak <= 0:
...
def shuffled_sample_index(completed_steps: int, sample_count: int, seed: int) -> int:
    """Map an optimizer-step count to a reproducible shuffled-epoch sample."""

    if completed_steps < 0:
        raise ValueError("completed_steps must be non-negative")
    if sample_count <= 0:
        raise ValueError("sample_count must be positive")
    epoch, offset = divmod(completed_steps, sample_count)
    order = np.random.default_rng(seed + epoch).permutation(sample_count)
...
def summarize_metric_map(
    ...
    worst_filename = max(
        metrics_by_filename,
        key=lambda filename: (
            metrics_by_filename[filename]["normalized_complex_rmse"],
            filename,
        ),
    )
    return {
...
def baseline_metrics(
    ...
    *,
    ...
    for sample in samples:
        zero[sample.filename] = evaluate_prediction(
        ...
        identity[sample.filename] = evaluate_prediction(
            sample.inputs,
            sample.targets,
            target_peak=sample.target_peak,
            floor_db=floor_db,
...
def sample_artifact_paths(paths: RunPaths, filename: str) -> RunPaths:
    stem = Path(filename).stem
    figures = paths.figures / stem
    predictions = paths.predictions / stem
...
def restore_checkpoint(
    ...
    *,
    ...
    if checkpoint.get("schema_version") != CHECKPOINT_SCHEMA_VERSION:
        raise RuntimeError("unsupported patch-set checkpoint schema version")
    if checkpoint.get("resolved_config") != resolved_config:
        raise RuntimeError("checkpoint configuration or sample set does not match")
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    print("selected patch set:", flush=True)
    for index, sample in enumerate(samples):
    ...
    def evaluate_and_record(mean_train_loss: float | None) -> dict[str, tuple[torch.Tensor, torch.Tensor]]:
    ...
    try:
        ...
        while step < args.steps and success_step is None:
            ...
            train_result = train_one_step(
                ...
                scheduler,
                scaler,
                {"input": sample.inputs, "target": sample.targets},
                device=device,
                precision=precision,
            ...
            overflow_streak = 0
            step += 1
            train_loss_sum += float(train_result.loss)
            train_loss_count += 1

            if step % args.eval_every == 0:
                mean_train_loss = train_loss_sum / train_loss_count
    ...
    if last_evaluation_step != step:
        mean_train_loss = (
            train_loss_sum / train_loss_count if train_loss_count > 0 else None
        )
        last_predictions = evaluate_and_record(mean_train_loss)
    if last_predictions is None:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Jointly overfit a deterministic set of mutually non-overlapping complex "
            "SAR patches with SwinIR."
        )
    )
