...
- 实验通过：局部 `512×512` Echo 幅度中包含可迁移到同场景未见区域的恢复信息；
- 训练误差下降但验证不优于基线：E003 主要是内容记忆，当前映射没有局部泛化；
- 大多数样本改善但强度塌缩：损失或输出参数化仍有问题，不能进入上下文结论；
- 指标稳定但未通过：保留最佳检查点，下一步进行更大输入、中心512输出的上下文消融；
- 本实验通过后：先人工审查最佳验证预测，再扩大到跨 parent scene 划分。
