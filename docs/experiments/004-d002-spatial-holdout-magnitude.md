...
E004 证明模型能在同场景未见区域中预测比 Echo identity 更接近标签的低频亮度包络：
441个验证 patch 的 RMSE 全部改善。但这一结果不能再表述为“结构泛化”；仅用逐像素
log-domain Charbonnier 会偏向大量低幅度背景，既不能保持强散射能量，也没有恢复
聚焦后的高频结构。
...
实际审查12个代表样本后，prediction 均主要表现为平滑低频团块。即使在各自峰值归一化
的上排，Image 中的道路、田块边界、线状强散射和细密结构仍未出现。因此 E004 在任务
层面判定为失败：它实现了低频回归和降噪，但没有实现散焦到聚焦的重聚焦。
