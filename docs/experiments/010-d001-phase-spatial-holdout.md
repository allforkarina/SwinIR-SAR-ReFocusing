...
训练在step 50000因连续10次验证没有改善而早停，`best.pt`仍停留在随机初始化的step 0。
最终验证集平均相位alignment约为0，Echo到Oracle的RMSE差距闭合比例约为0，全部预设门槛
均未通过（只有高频能量median仍在允许范围）。

修正审查脚本后，使用`final.pt`和其同step的`last_validation`导出了step 50000图像。多个代表
样本的Raw结果没有形成Oracle和Image中的道路、建筑及地块结构，只剩错误相位干涉形成的
暗背景、弥散纹理和少数亮点。由于单位相位输出保持Echo频谱幅度与总能量，该现象是结构性
相位崩溃，不是数值输出趋零。

E010因此只能确定“step 50000没有在空间留出验证区学习到可识别的重聚焦映射”。它不能区分
模型是否在训练样本上发生记忆。下一步进入E011-A，对同一E010模型的step 0与step 50000
checkpoint进行训练集probe对照，不进行额外训练。
