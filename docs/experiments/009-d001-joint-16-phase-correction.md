...
Raw模型在step 40000达到连续三次`16/16`通过并提前停止。集合最差指标仍满足全部预设
门槛：

| 集合指标 | step 40000结果 | 门槛 |
| --- | ---: | ---: |
| 最低weighted phase alignment | 0.9764 | ≥ 0.95 |
| 最大Oracle RMSE差 | 0.0166 | ≤ 0.08 |
| 最低Oracle coherence比例 | 0.9829 | ≥ 0.90 |
| 最低Oracle SSIM增益比例 | 1.1257 | ≥ 0.80 |
| 最低Oracle edge增益比例 | 1.1931 | ≥ 0.75 |
| 高频能量ratio范围 | 0.8805～1.0152 | 0.75～1.25 |

项目组抽查了多个不同坐标的五列双尺度图。Raw和预热后的EMA都恢复出与Oracle一致的道路、
地块边缘和散射结构，不是E004/E005中仅改变幅度、降噪或增强对比度的结果。人工审查接受
“16个训练样本均完成重聚焦”的结论。

该结果排除了“模型连16个样本都无法共同表示相位校正”的假设，但模型参数仍可能只记忆
这16个Image标签。E009不提供泛化证据；下一步必须在从未参与训练且与训练patch无重叠的
空间区域执行E010。
